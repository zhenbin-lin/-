...
//            LogUtil.w(Tag,getClass().getSimpleName() + " getDataLists  mMediaPlayer is null");
            return null;
        }
        if (mMediaPlayer instanceof IjkMediaPlayer) {
            mediaPlayer = (IjkMediaPlayer) mMediaPlayer;
        } else if (mMediaPlayer instanceof MediaPlayerProxy) {
            //todo 目前没走到，使用的是IjkMediaPlayer
...
//        LogUtil.i(getClass().getSimpleName() + " updateLoadCost " + time);
        mLoadCost = time;
    }

    public void updateSeekCost(long time) {
//        LogUtil.i(getClass().getSimpleName() + " updateSeekCost " + time);
        mSeekCost = time;
    }
